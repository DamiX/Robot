...
(* sera utile plus tard, lors de la creation du systeme de score *)
...
let nouveau_niveau n_lvl n_teleport = 
  let lobjets = creer_robots (n_lvl*nb_robots) in
   { objets = lobjets;
...
(* dont le second appel ne respectera pas les conditions             *)
(* /!\ WARNING /!\ : Teleport peux teleporter a coté d'un robot      *)
(* puis mourir => OK... D'ou la teleportation sure                   *)
...
(* TODO : revoir la facon de compter les robots detruits       *) 
...
let tour lvl contrll =
  let apres_coup = coup lvl contrll in
   if(gagne apres_coup) then raise (Gagne apres_coup.nb_teleportation)
   else
     begin
       if(perdu apres_coup) then
	 begin
...
let rec jouer contrll lvl =
  ...
  wait(180.0); (* Attente entre chaque input *)
  jouer contrll (tour lvl contrll)

let rec jeu n_lvl clavier n_teleport =
  try jouer clavier (nouveau_niveau n_lvl n_teleport) with
  |Gagne n -> begin
               bg_transition (String.concat " " (["Niveau"; string_of_int (n_lvl+1)]));
               wait(850.0);
               jeu (n_lvl+1) clavier n;
	      end
  |Perdu -> begin
             ...
             while 1=1 do
              let capture = wait_next_event([Mouse_motion]) in
              if(is_collision_box (557,755) (365,408) capture.mouse_x capture.mouse_y) then
               begin
